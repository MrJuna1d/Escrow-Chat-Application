...
import { QR_ECL_PERS } from "../constants.js";
import { consoleWarn } from "../helpers.js";
export default function useQRCodeLogoSize({
  ...
  const [transformedLogoSize, setTransformedLogoSize] = useState({
    width: 0,
    height: 0
  });
  const transformLogoSize = useCallback((width, height) => {
    ...
    if (logoHref) {
      ...
      if (typeof logoHref === 'number') {
        const {
          width,
          height
        } = Image.resolveAssetSource(logoHref);
        transformLogoSize(width, height);
      }

      // if logo is string & remote
      if (typeof logoHref === 'string') {
        Image.getSize(logoHref, transformLogoSize, () => consoleWarn('Something wrong with logo url: ' + logoHref));
      }

      // if logo is object & remote
      if (typeof logoHref !== 'string' && typeof logoHref !== 'number' && !Array.isArray(logoHref) && logoHref.uri) {
        Image.getSize(logoHref.uri, transformLogoSize, () => consoleWarn('Something wrong with logo url: ' + logoHref.uri));
      }
    }
