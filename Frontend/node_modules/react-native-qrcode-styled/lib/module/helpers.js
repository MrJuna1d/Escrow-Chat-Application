...
import { INNER_EYE_SIZE_IN_BITS, EYES_POSITIONS, OUTER_EYE_SIZE_IN_BITS } from "./constants.js";
export function convertMultiValueToNumber(value, unit) {
...
export function convertBorderRadiusMap(borderRadiusMap, unit) {
...
export function transformEyeOptionsToCommonPattern(options) {
  ...
  return EYES_POSITIONS.reduce((res, position) => ({
    ...res,
    [position]: options
  }), {});
}
...
export function getPieceRoundedSquarePathData({
  ...
  let [topLeftR = 0, topRightR = 0, bottomRightR = 0, bottomLeftR = 0] = borderRadius ? convertBorderRadiusMap(borderRadius, size) : [];
...
export function getRoundedOuterEyePathData(position, borderRadius, pieceSize, qrSize) {
  ...
  let [topLeftR = 0, topRightR = 0, bottomRightR = 0, bottomLeftR = 0] = convertBorderRadiusMap(borderRadius, pieceSize * 7) || [];
...
export function getRoundedInnerEyePathData(position, borderRadius, pieceSize, qrSize) {
  ...
  const [topLeftR = 0, topRightR = 0, bottomRightR = 0, bottomLeftR = 0] = convertBorderRadiusMap(borderRadius, pieceSize * 3) || [];
  if (position === 'topLeft') {
    return `
