...
import { getInnerEyePathData, getOuterEyePathData, getPieceLiquidPathData, getPieceRoundedSquarePathData, getPieceSquarePathData, getRoundedInnerEyePathData, getRoundedOuterEyePathData, isCoordsOfInnerEyes, isCoordsOfOuterEyes, isLiquidPieceInEyes, transformBorderRadiusToArray } from "../helpers.js";
  ...
  for (let y = 0; y < bitMatrix.length; y++) {
    for (let x = 0; x < bitMatrix.length; x++) {
      ...
      if (bitMatrix[y]?.[x] === 1) {
        ...
        let PathComponent = /*#__PURE__*/_jsx(Path, {
          scale: pieceScale,
        ...
        if (outerEyesOptions || innerEyesOptions) {
          if (outerEyesOptions && !innerEyesOptions && !isCoordsOfOuterEyes(x, y, bitMatrix.length) || !outerEyesOptions && innerEyesOptions && !isCoordsOfInnerEyes(x, y, bitMatrix.length) || innerEyesOptions && !isCoordsOfInnerEyes(x, y, bitMatrix.length) && outerEyesOptions && !isCoordsOfOuterEyes(x, y, bitMatrix.length)) {
        ...
        if (pieceLiquidRadius) {
          if (outerEyesOptions && !isLiquidPieceInEyes(x, y, bitMatrix.length) || !outerEyesOptions) {
            const d = getPieceLiquidPathData(x, y, pieceSize, pieceLiquidRadius);
            const origin = `
            ${x * pieceSize + pieceSize / 2},
            ${y * pieceSize + pieceSize / 2}`;
            if (bitMatrix[y]?.[x - 1] === 1 && bitMatrix[y - 1]?.[x] === 1) {
              svgPiecesMatrix.push(/*#__PURE__*/_jsx(Path, {
                d: d
              }, `${x}_${y}_topLeft`));
            }
            if (bitMatrix[y]?.[x - 1] === 1 && bitMatrix[y + 1]?.[x] === 1) {
  ...
  if (outerEyesOptions) {
    ...
    listPositions.forEach(position => {
      ...
      if (Object.keys(outerEyesOptions).includes(position)) {
        const transformedOuterEyeBorderRadius = transformBorderRadiusToArray(outerEyesOptions[position]?.borderRadius);
  ...
  if (innerEyesOptions) {
    ...
    listPositions.forEach(position => {
      ...
      if (Object.keys(innerEyesOptions).includes(position)) {
        const transformedInnerEyeBorderRadius = transformBorderRadiusToArray(innerEyesOptions[position]?.borderRadius);
...
export default /*#__PURE__*/memo(SVGPiecesImpl, isEqual);
