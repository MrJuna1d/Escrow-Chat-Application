...
exports.getInnerEyePathData = getInnerEyePathData;
exports.getOuterEyePathData = getOuterEyePathData;
exports.getPieceLiquidPathData = getPieceLiquidPathData;
exports.getPieceRoundedSquarePathData = getPieceRoundedSquarePathData;
exports.getPieceSquarePathData = getPieceSquarePathData;
exports.getRoundedInnerEyePathData = getRoundedInnerEyePathData;
exports.getRoundedOuterEyePathData = getRoundedOuterEyePathData;
exports.isCoordsOfBottomLeftInnerEye = isCoordsOfBottomLeftInnerEye;
exports.isCoordsOfBottomLeftOuterEye = isCoordsOfBottomLeftOuterEye;
exports.isCoordsOfInnerEyes = isCoordsOfInnerEyes;
exports.isCoordsOfOuterEyes = isCoordsOfOuterEyes;
exports.isCoordsOfTopLeftInnerEye = isCoordsOfTopLeftInnerEye;
exports.isCoordsOfTopLeftOuterEye = isCoordsOfTopLeftOuterEye;
exports.isCoordsOfTopRightInnerEye = isCoordsOfTopRightInnerEye;
exports.isCoordsOfTopRightOuterEye = isCoordsOfTopRightOuterEye;
exports.isLiquidPieceInEyes = isLiquidPieceInEyes;
exports.transformBitArrayToMatrix = transformBitArrayToMatrix;
exports.transformBorderRadiusToArray = transformBorderRadiusToArray;
exports.transformEyeOptionsToCommonPattern = transformEyeOptionsToCommonPattern;
var _constants = require("./constants.js");
function transformBitArrayToMatrix(bitArray, qrCodeSize) {
  const matrix = [];
  let row = [];
...
function transformBorderRadiusToArray(borderRadius) {
  if (!borderRadius) {
    return undefined;
  }
...
function transformEyeOptionsToCommonPattern(options) {
  if (!options) {
    return undefined;
  }
  if (Object.keys(options).find(key => _constants.EYES_POSITIONS.includes(key))) {
    return options;
  }
  return _constants.EYES_POSITIONS.reduce((res, position) => ({
    ...res,
    [position]: options
  }), {});
}
...
function getPieceSquarePathData(x, y, size) {
  const _x = x * size;
  const _y = y * size;
  return `
    M${_x} ${_y}
...
// x, y - indexes in matrix
function getPieceRoundedSquarePathData({
  x,
  y,
  size,
  cornerType,
...
function getPieceLiquidPathData(x, y, size, borderRadius) {
  const _x = x * size;
  const _y = y * size;
...
function getOuterEyePathData(position, pieceSize, qrSize) {
...
function getInnerEyePathData(position, pieceSize, qrSize) {
  const outerSize = _constants.OUTER_EYE_SIZE_IN_BITS * pieceSize;
  const innerSize = _constants.INNER_EYE_SIZE_IN_BITS * pieceSize;
...
function getRoundedInnerEyePathData(position, borderRadius, pieceSize, qrSize) {
  const outerSize = _constants.OUTER_EYE_SIZE_IN_BITS * pieceSize;
  const innerSize = _constants.INNER_EYE_SIZE_IN_BITS * pieceSize;
  const offset = 2 * pieceSize;
...
function isLiquidPieceInEyes(x, y, qrSize) {
  return (
    // top left square
    x >= 1 && x < 6 && y >= 1 && y < 6 ||
    // top right square
...
function isCoordsOfTopLeftOuterEye(x, y) {
  return x >= 0 && x < _constants.OUTER_EYE_SIZE_IN_BITS && y === 0 || x >= 0 && x < _constants.OUTER_EYE_SIZE_IN_BITS && y === _constants.OUTER_EYE_SIZE_IN_BITS - 1 || y > 0 && y < _constants.OUTER_EYE_SIZE_IN_BITS - 1 && x === 0 || y > 0 && y < _constants.OUTER_EYE_SIZE_IN_BITS - 1 && x === _constants.OUTER_EYE_SIZE_IN_BITS - 1;
}
function isCoordsOfTopRightOuterEye(x, y, qrSize) {
  return x >= qrSize - _constants.OUTER_EYE_SIZE_IN_BITS && x < qrSize && y === 0 || x >= qrSize - _constants.OUTER_EYE_SIZE_IN_BITS && x < qrSize && y === _constants.OUTER_EYE_SIZE_IN_BITS - 1 || y > 0 && y < _constants.OUTER_EYE_SIZE_IN_BITS - 1 && x === qrSize - _constants.OUTER_EYE_SIZE_IN_BITS || y > 0 && y < _constants.OUTER_EYE_SIZE_IN_BITS - 1 && x === qrSize - 1;
}
function isCoordsOfBottomLeftOuterEye(x, y, qrSize) {
  return x >= 0 && x < _constants.OUTER_EYE_SIZE_IN_BITS && y === qrSize - _constants.OUTER_EYE_SIZE_IN_BITS || x >= 0 && x < _constants.OUTER_EYE_SIZE_IN_BITS && y === qrSize - 1 || y > qrSize - _constants.OUTER_EYE_SIZE_IN_BITS && y < qrSize && x === 0 || y > qrSize - _constants.OUTER_EYE_SIZE_IN_BITS && y < qrSize && x === _constants.OUTER_EYE_SIZE_IN_BITS - 1;
}

// x, y is amount of matrix bits
function isCoordsOfOuterEyes(x, y, qrSize) {
  return (
    // top left square
...
function isCoordsOfTopLeftInnerEye(x, y) {
  return x >= 2 && x < _constants.INNER_EYE_SIZE_IN_BITS + 2 && y >= 2 && y < _constants.INNER_EYE_SIZE_IN_BITS + 2;
}
function isCoordsOfTopRightInnerEye(x, y, qrSize) {
  return x >= qrSize - _constants.OUTER_EYE_SIZE_IN_BITS + 2 && x < qrSize - 2 && y >= 2 && y < _constants.INNER_EYE_SIZE_IN_BITS + 2;
}
function isCoordsOfBottomLeftInnerEye(x, y, qrSize) {
  return x >= 2 && x < _constants.INNER_EYE_SIZE_IN_BITS + 2 && y >= qrSize - _constants.OUTER_EYE_SIZE_IN_BITS + 2 && y < qrSize - 2;
}

// x, y is amount of matrix bits
function isCoordsOfInnerEyes(x, y, qrSize) {
  return (
    // top left square
